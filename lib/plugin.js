import Vue from 'vue'
import Segment from './vue-segment'
...
async function getSegmentPageData(to) {
  // Get the segment metadata for the page
  let publishedVersion = ''
  let publishedAt = ''
  // Try and get the additional data from the quibbler
  try {
    // Try and get the url at the end, as a last resort if we don't have any params
    const revisions = document.querySelector('[data-contentful-revisions]')?.dataset?.contentfulRevisions
    if(revisions){
      // Parse the json for the revisions
      const revisionJson = JSON.parse(revisions)
      if(revisionJson){
        // Get the published version and published at
        publishedVersion = revisionJson?.publishedVersion
        publishedAt = revisionJson?.publishedAt
      }
    }
  } catch (e) {}
