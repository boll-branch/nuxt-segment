...
function moduleOptionRouter(moduleOptions) {
  return checkIfBoolean(moduleOptions.useRouter)
    ? moduleOptions.useRouter
    : undefined;
}

function configOptionRouter($config) {
  return checkIfBoolean($config.SEGMENT_USE_ROUTER)
...
function shouldUseRouter(moduleOptions, $config) {
  return (
    moduleOptionRouter(moduleOptions) ||
    configOptionRouter($config) ||
    DEFAULT_USE_ROUTER
  );
}
...
function getSegmentMetadata() {
  ...
  const iterableCampaignId = Cookies.get("iterableEmailCampaignId");
  const iterableTemplateId = Cookies.get("iterableTemplateId");
  const testId = Cookies.get("_bb_test_id");
  let userID = "";
  let email = "";
  if (userData) {
...
export default function (context, inject) {
  ...
  const options = {
    ...
    disabled:
      moduleOptions.disabled || ($config && $config.SEGMENT_DISABLED) || false,
