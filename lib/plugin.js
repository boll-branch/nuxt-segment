...
async function getSegmentPageData(to) {
  ...
  try {
    ...
    const parsedHandle = to.path.split("/").filter(Boolean).pop()
    const url =  to?.name === 'index' ? 'homepage' : to?.params?.pageHandle || to?.params?.collectionHandle || to?.params?.productHandle || parsedHandle
    const contentCache = await fetch(
      quibblerURL + '/api/content/' + url,
      {
        headers: {
