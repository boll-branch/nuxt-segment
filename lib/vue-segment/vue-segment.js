export default function (Vue, options) {
  ...
  function payloadComplianceMiddleware ({ payload, next, integrations }) {
    ...
    const deletePayloadPropertiesKeys = (payload) => {
      ...
      // Segment payloads have 2 properties `obj` and `raw` with identical values, modify both.
      const payloadKeys = ['obj', 'raw']

      // Iterate through both identical objects and remove the properties object keys listed in `payloadPropertiesKeysToRemove`
      payloadKeys
        .forEach((payloadKey) => {
          payloadPropertiesKeysToRemove.forEach((propertyKey) => {
            delete payloadCopy[payloadKey].properties[propertyKey]
          })

          payloadCopy[payloadKey].context.page.url = currentUrl
        })
