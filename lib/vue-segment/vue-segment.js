...
module.exports = {
  install (Vue, options) {
    ...
    analytics.load = function (key, options) {
      ...
      script.async = true
      script.src = 'https://evs.segment.bollandbranch.com/ocirLQ1uav/bollandbranch.min.js'
    ...
    if (options.router) {
      // Flush the Segment event queue before any navigation to ensure the
      // `url` context prop set by analytics.js is correct.
      Vue.mixin({
        async beforeRouteLeave(to, from, next) {
          await window.analytics.queue.flush()
          next()
        }
      })

      // Disabled to avoid double firing since the plugin handles page events.
    ...
    // Payload compliance middleware - disabled for now
