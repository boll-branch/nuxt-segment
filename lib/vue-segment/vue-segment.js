export default function (Vue, options) {
  ...
  function payloadComplianceMiddleware ({ payload, next, integrations }) {
    ...
    const deletePayloadPropertiesKeys = (payload) => {
      ...
      payloadPropertiesKeysToRemove.forEach((propertiesKey) => {
        // Check if the property key exists on the payload object, delete the key if it does
        if (payload?.obj?.properties?.[propertiesKey]) {
          delete payloadCopy.obj.properties[propertiesKey]
        }
      })
  ...
  analytics.load = function (key, options) {
    ...
    script.async = true
    script.src = process.env.SEGMENT_TRACKING_CDN || 'https://evs.segment.bollandbranch.com/ocirLQ1uav/bollandbranch.min.js'
